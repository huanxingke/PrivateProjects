...
class Client:
    ...
    def upload_content(self, content: bytes, to_path: str) -> None:
        if not isinstance(content, bytes):
            raise Exception("Content must be bytes")
        http_resp = self.request(HTTPMethod.PUT, to_path, content=content)
        http_resp.raise_for_status()

    def get_content(self, path: str) -> str:
        if self.isdir(path):
            raise IsACollectionError(path, "Cannot open a collection")
        with IterStream(self, self.join_url(path)) as buffer:
            buff = buffer.read()
            return buff
